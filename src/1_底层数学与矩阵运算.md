...
2. 如何在尽可能内存的前提下操作 $\mathbb{F}_2^{n \times m}$（$n \approx 10^4, m \approx 10^5$）量级的二元矩阵，这是基于编码的 ZK 协议需要大量的随机矩阵（如 $B \in \mathbb{F}_2^{n \times m}$），直接放进去会导致大量的内存占用。
...
```mermaid
graph TD
    subgraph SMC [StreamingMatrixCols 核心逻辑]
        direction TB
    ...
    %% 样式优化：去掉了刺眼的亮色背景，改为只使用彩色边框（完美适配暗色/亮色模式）
    style SMC fill:transparent,stroke:#666,stroke-width:2px,stroke-dasharray: 5 5
    style E fill:transparent,stroke:#e06c75,stroke-width:3px  %% 红色边框突出生成器
    style G fill:transparent,stroke:#61afef,stroke-width:3px  %% 蓝色边框突出最终产物
```

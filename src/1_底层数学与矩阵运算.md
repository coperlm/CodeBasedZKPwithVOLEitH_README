...
加法即按位 XOR；乘法是多项式乘法再模实现所选的 128 次不可约多项式。具体表示由 `swanky_field` 的 `F128b` 实现决定，本文不把底层表示常数当作协议参数。
...
`benches/gf128_bench.rs` 可用于测量域乘法；具体耗时随 CPU、编译参数和线程环境变化，不能把某次微基准数字写成协议性质。

### 2.3 安全注意事项

有限域函数的实现路径避免了显式的秘密相关分支，但完整侧信道安全仍取决于编译器、硬件、调用上下文和整个协议，不能仅由使用硬件指令推出。
...
矩阵默认不完整存储在内存中。`StreamingMatrixCols` 保存 16 字节种子、维度、尾部掩码和 PRG 状态；每次调用 `column(j)` 生成一列的 `n` 个比特，用完即丢弃。因此主要工作空间从物化矩阵的 $\mathcal{O}(nm)$ 降为一列缓冲区和 PRG 状态，具体字节数随硬件后端而变。

...
2. 基于编码的 ZK 协议需要大量的随机矩阵（如 $B \in \mathbb{F}\_2^{n \times m}$），如何快速操作这类大规模二元矩阵，并改善缓存局部性。
...
代码没有自己实现域运算，而是复用 `swanky` 的有限域接口：

固定依赖 `swanky-field-binary` 将域元素表示为 128 位多项式系数，并以无进位乘法和模 $x^{128}+x^7+x^2+x+1$ 的折叠约简实现乘法。目标平台可由底层 SIMD 实现利用 PCLMULQDQ 或 PMULL；AES-NI 则用于本项目的矩阵 PRG。

`benches/gf128_bench.rs` 用于单独测量域乘法；本说明的性能结论采用模块十记录的端到端签名与验证时间。
...
若直接物化，该单个矩阵约为 $1280 \times 40960\text{ bits}=6.25\text{ MiB}$（不计容器开销）。上层关系会在求值时反复访问这类矩阵；完整保存多张矩阵会增大内存占用并降低缓存局部性。应用层的种子派生矩阵与 FAEST 的 VOLE/BAVC 树是不同对象：前者服务于 AFS 哈希和线性关系，后者服务于 VOLE 承诺与打开。

本实现采用列式伪随机流式生成，不存储整个矩阵，而是存储一个 128-bit AES key、一个 128-bit IV 和维度。论文对齐矩阵的 IV 为零；Scope-LRS `ChosenScope256` 允许非零 IV。某一列仍只在被请求时即时计算。

### 3.2 核心数据结构与公式-代码映射

| 数学符号 | 代码对应 | 说明 |
|---------|---------|------|
| $B \in \mathbb{F}\_2^{n \times m}$ | `StreamingMatrixCols` | 不存储矩阵本身，仅持有 AES key/IV 和维度信息 |
| 种子/AES key $\sigma$ | `seed: [u8; 16]` | 128-bit AES key，用于确定性扩展 |
| 初始计数器 $\nu$ | `iv: [u8; 16]` | 论文对齐路径为零；256-bit 派生路径使用后 128 bit |
...
graph TD
    subgraph SMC [StreamingMatrixCols<br>核心逻辑]
        direction TB
        A[key: 16 Bytes<br>IV: 16 Bytes] -->|初始化 MatrixPrg| B(AES-128 密钥扩展)
        B --> C[(round_keys 11轮子密钥)]
        C -->|全局复用密钥| E{AES-CTR 生成器}
        Req[外部按需请求 column j] -.->|传入 col_idx = j| E
        E --> |"upper word += column j<br>low word += block index"| F[fill_column_bytes]
...
矩阵默认不完整存储在内存中。`StreamingMatrixCols` 保存 16 字节 key、16 字节 IV、维度、尾部掩码和 PRG 状态；每次调用 `column(j)` 生成一列的 $n$ 个比特，完成计算后释放临时列数据。因此主要工作空间从物化矩阵的 $\mathcal{O}(nm)$ 降为一列缓冲区和 PRG 状态，具体字节数随所选实现路径而变。这只是矩阵物化的空间边界，不是整个签名过程的 peak-RSS $\mathcal{O}(1)$ 声称。

#### 策略二：延迟生成

`fill_column_bytes` 使用按需填充模式。只有 `column(j)` 或 `fill_column_bytes` 被显式调用时，数据才被生成。`materialize()` 提供完整矩阵的参考接口，主要用于测试和等价性验证；在线签名与验证的主要路径通过列缓冲区逐列处理矩阵。

#### 策略三：AES-CTR 作为向量 PRG

每一列 $B_{:,j}$ 通过 AES-128 在 FAEST PRG 的 CTR 布局下生成。记 $\nu[j,k]$ 为在公开 IV $\nu$ 的高 32-bit little-endian word 上加列 tweak $j$，并在低 32-bit word 上加块序号 $k$ 得到的计数器，则：

$$
B_{:,j} = \text{AES}\_\sigma(\nu[j,0]) \; \| \; \text{AES}\_\sigma(\nu[j,1]) \; \| \; \cdots
$$

其中 $\sigma$ 是 16 字节 AES key。旧构造器固定 $\nu=0$，新的 256-bit 材料构造器将前/后 16 字节分别用作 $\sigma/\nu$。每个 AES 块产生 128 位伪随机输出，因此生成 $n$ 位列向量需要 $\lceil n/128 \rceil$ 次 AES 加密调用。
...
测试代码 `test_matrix_vector_multiply_report` 提供了 1280 行、81920 列、汉明重量 320 的参考测量配置。该矩阵运算被纳入模块十的端到端签名与验证计时。
...
这三种模式服务于不同的工程目的：行主序用于验证正确性，非 CT 列流式用于公开输入的快速求值，CT 列流式用于秘密输入的侧信道防御。
...
| 操作 | 实现方式 | 性能特征 |
|------|---------|---------|
| $\mathbb{F}\_{2^{128}}$ 乘法 | CLMUL/PMULL 硬件指令 | ~0.5 ms / 万次|
| 矩阵列生成（$n=1280$） | AES-CTR, 约 10 个 AES 块 | < 1 μs / 列 |
| $y = Bx$ 稀疏乘法（$w=320$） | 字节级 XOR 累加 | < 50 ms / 次 |
| 矩阵全物化 | 在主要在线路径不发生 | 从 $\mathcal{O}(nm)$ 物化降为 $\mathcal{O}(n)$ 列缓冲 + $\mathcal{O}(1)$ 描述符/PRG 状态；不声称整个过程 peak RSS 为常数 |

...
128-bits 域运算与大矩阵流式计算

## 一、概述

本模块是整个 ZKP 系统的基石，也是性能瓶颈突破口。它解决两个根本问题：
1. 如何在 $\mathbb{F}\_{2^{128}}$ 上尽可能快的做乘法运算，这一点同样也在原论文被提及到。
2. 如何在更快速地操作 $\mathbb{F}\_2^{n \times m}$（$n \approx 10^4, m \approx 10^5$）量级的二元矩阵，基于编码的 ZK 协议需要大量的随机矩阵（如 $B \in \mathbb{F}\_2^{n \times m}$），同时增大 cache 命中率。
...
加法即按位 XOR；乘法是多项式乘法再模一个 128 次不可约多项式 $x^{128} + x^7 + x^2 + x + 1$。
...
从 `benches/gf128_bench.rs` 可以看到，当使用 `RUSTFLAGS="-C target-cpu=native"` 编译时，微基准测试报告显示平均每次乘法耗时在亚纳秒级（ < 0.5 ms / 万次），验证了硬件加速被正确激活。

### 2.3 安全注意事项

域乘法的常数时间性质由硬件指令（PCLMULQDQ / PMULL）天然保证，这些指令固定延迟的，也没有任何内存分配或可变状态操作，因此不受侧信道攻击的风险。
...
若直接物化，$1280 \times 81920 \text{ bits}\approx 12.5 \text{ MB}$。对于单张矩阵尚且可控，但协议运行中会生成许多张这样的矩阵，加上中间变量的暂存，一方面会增大内存的占用，另一方面会降低 cache 的命中率。但是VOLE 树的叶子承诺需要用到种子衍生的矩阵只在求值时用一次，无需常驻内存。

所以最佳实现方案是列式伪随机流式生成。我们不存储整个矩阵，而是存储一个 128-bit 种子，在需要某一列时即时计算。
...
矩阵从未完整存储在内存中。`StreamingMatrixCols` 仅持有 32 字节的状态信息（16 字节种子 + 部分元数据）。每次调用 `column(j)` 生成一列的 `n` 个比特，用完即丢弃。这使得 $1280 \times 81920$ 的矩阵实际占用从约 $12.5\text{ MB}$ 降至 $32\text{ B}$。

#### 策略二：延迟生成

`fill_column_bytes` 使用按需填充模式。只有 `column(j)` 或 `fill_column_bytes` 被显式调用时，数据才被生成。`materialize()` 方法虽然提供了全矩阵物化的接口用于测试，但在正式实验的代码中不使用，正式实验只使用 `iter()` 逐个消费列。
...
在基于编码的 ZK 中，$y = B \cdot x$ 中 $x$ 是一个正则编码词，其汉明重量极低，$x$ 中只有 $c$ 个比特为 1，其余全为 0。这意味着全矩阵乘法可以退化为稀疏乘法：

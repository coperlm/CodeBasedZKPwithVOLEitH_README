...
代码还没完成的部分：
1. setup 和 签名阶段分开；
2. LRS 可链接性判定，群签名的 GOpen，FDABS的Update / Revoke（属性撤销/时代更新）
3. 理论上还有更多变体可以构建，例如多权威机构MAABS，可问责环签名等

本实现底层采用图灵完备的 NAND 约束，上层需进行策略电路编译

上述阶段你加入了一些更为细粒度的阶段，就像初始化和撤销阶段等，请你修改性能测试的部分（可能是 `benches/paper_benchmarks.rs`），我希望得到更全面的性能测试结果，进行十次测试（去掉一次最大值和最小值），并输出一个完整的表格为我展现最终的测试结果。最终的呈现格式我希望形似 代码架构说明/src/10_原始数据.md 里面的 端到端结果 一节
...
本模块把论文 §3 的正则编码代数化方法落实为约束编译和隐式求值代码。它解决的核心问题是：

> 如何将正则编码函数（Regular Encoding），即一个看似非线性的、基于查表的操作，转化为 degree-$c$ 的布尔多项式（ANF），然后在 ZK 协议的"隐式空间"中高效地安全求值？

这分为两个阶段：

1. `constraint_gen.rs` — 编译阶段：离线预计算正则编码的 ANF 系数表。论文 §3.2 将每个正则编码输出位表示为次数至多为 $c$ 的布尔多项式。
2. `implicit_eval.rs` — 求值阶段：在线计算 χ-加权隐式多项式，利用求和交换和蝴蝶折叠减少重复的 ANF 求值；实际循环开销按块大小 $2^c$ 计量。
...
完整的 ZK 协议需要 Prover 证明约束系统 $\{g_i(Y) = 0\}\_{i=1}^t$ 成立。逐个对 $t$ 个约束多项式求值和插值的开销为 $\mathcal{O}(t \cdot 2^c)$；在 $t \approx 1280$、$c \approx 8 \sim 16$ 时，求和交换可显著减少重复计算。
...
利用这个交换律，可以先把约束索引 $i$ 的求和折叠成列系数 $\chi'_j$，从而避免对每个约束重复扫描同一组 regular-encoding 项。之后的 `evaluate_chunk_butterfly` 对每个大小为 $2^c$ 的块做逐层折叠；其工作量由实际块数和 $2^c$ 决定。
...
这利用最低有效位递推：由 `mask & (mask-1)` 移除一个置位比特，再将相应挑战值 XOR 回去。`subset_table[mask]` 因而保存该掩码所选挑战值的 XOR。一旦预计算完成，任何列的求值退化为 $n/8$ 次查表加 XOR：
...
并行化：当列数 $\ge 8192$ 时，自动启用 `rayon` 并行，每 256 列构成一个分块任务；每个工作线程使用独立的 `column_bytes` 缓冲区。
...
蝴蝶算法将 $2^c$ 个输入系数组织为 $c$ 层原地折叠；每层的元素数减半，每个块的总工作量为 $\mathcal{O}(2^c)$。它通过复用中间结果减少逐项求值，结构与 FFT 的 Cooley-Tukey 蝶形相似：
...
栈优化：当 $2^c \le 256$ 时，使用栈分配的定长数组 `[F; 256]` 代替堆分配的 `Vec`，避免性能关键循环中的动态内存分配。
...
1. 选择 c+1 个插值点 $p\_0,\ldots,p\_c$；实现取 $0,1,g,g^2,\ldots$，其中 $g=\texttt{F::GENERATOR}$
2. 对每个插值点 p_k:
...
| 优化技术 | 位置 | 加速比（相对朴素实现） |
|---------|------|---------------------|
| 求和交换（χ' 聚合） | `compute_chi_prime` | 把约束维度的重复求和改为列聚合 |
| 字节子集 XOR 表 | `build_byte_subset_xor_table` | 以查表和 XOR 替代逐位扫描 |
| 手工 160 字节展开 | `xor_32!` / `subset_xor_from_bytes` | 减少循环控制开销 |
| 栈分配工作缓冲区 | `evaluate_chunk_butterfly`（$2^c \le 256$） | 消除堆分配 |
| Rayon 并行 | 由输入规模和线程数触发 | 当前 16 工作线程实验的端到端结果见模块十 |
| 配对蝴蝶同时求值 | `evaluate_pair_*_butterfly` | 合并左右矩阵的求值流程 |
...
| 独热函数 $f\_{\text{idx}}$ | `table[idx]`（ANF 系数行） |
| 聚合系数 $\chi'\_j = \sum\_i \chi\_i b\_{i,j}$ | `compute_chi_prime(challenges, b_cols)` |

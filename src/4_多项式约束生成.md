...
2. `implicit_eval.rs` — 求值阶段：在线计算 χ-加权隐式多项式，利用求和交换和蝴蝶折叠减少重复的 ANF 求值；具体循环仍需按块大小 $2^c$ 计量，不能简单写成与 $c$ 线性。
...
利用这个交换律，可以先把约束索引 $i$ 的求和折叠成列系数 $\chi'_j$，从而避免对每个约束重复扫描同一组 regular-encoding 项。之后的 `evaluate_chunk_butterfly` 对每个大小为 $2^c$ 的块做逐层折叠；它的工作量应按实际块数和 $2^c$ 计量，而不是笼统宣称为 $\mathcal{O}(m\cdot c)$。
...
蝴蝶算法仍然需要处理输入的 $2^c$ 个系数，但把求值组织为 $c$ 层原地折叠；每层的元素数减半，总工作量仍应按 $\mathcal{O}(2^c)$（每个块）理解。它的优势在于避免逐个输出多项式、单项式地重复求值，而不是把指数规模变成线性规模。它的结构与 FFT 的 Cooley-Tukey 蝶形相似：

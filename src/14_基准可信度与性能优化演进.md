...
> **版本与证据边界。** 本章的静态代码基线是 `03a8be9b53a08e575627d5096f103f9fd2b9c2ec`，中间节点是 `3fbea395a3f62991414d06abd7549bdef3d8ec3a`，已提交审查终点是 `93b42e12684e9f919c873bad710f67db497603a5`。三者位于同一条一阶父提交链上。第十三节继续审查以 `93b42e1` 为 HEAD 的 2026-08-03 未提交 SIMD/native-C 工作树；对应最终性能可执行文件的 `source-worktree.patch` SHA-256 为 `c971dec55454fd1698716975d6a37735b1231a7c3ab6d8a63cb805eb47f07d40`。第十四节再审查外层仓库以 `e041b0f` 为 HEAD 的 Scope-LRS 双 profile 工作树，性能数据是明确允许 dirty worktree 的开发轮，不是可直接引用的 clean-commit 发布数据。本章的文档补写发生在测量之后，不属于被测二进制。代码 diff、历史提交说明、论文 PDF 和同机 A/B 实测是四类不同证据；只有同机 A/B 数据用于给出性能倍率，不能用静态代码行数推导速度。
...
## 十四、2026-08-03：Scope-LRS 双矩阵 profile 与 `justfile` 性能入口收口

### 14.1 为什么保留两种 profile

此次没有把“128 bit 足够”和“攻击者可选 scope”强行合并成一个模糊方案，而是保留两个显式的 `ScopeMatrixProfile`：

| Profile | scope 模型 | 公开矩阵派生材料 | 可以声称的边界 |
|---|---|---|---|
| `TrustedScope128` | scope 由可信方规范发放，命名空间有界，不允许攻击者自适应搜索任意 pair | domain-separated Keccak digest 的前 128 bit 作 AES-128 key，IV 为零 | 接受 $q$ 个已发放 scope 约 $q(q-1)/2^{129}$ 的 seed 碰撞概率；不声称抵抗攻击者的任意 pair 生日搜索 |
| `ChosenScope256` | caller/attacker 可选 scope | 完整 256-bit digest 拆成公开 AES-128 key 与 128-bit IV | 消除“先截断到 128-bit seed”造成的直接 $2^{64}$ 生日瓶颈；不等价于已证明 chosen-scope unlinkability |

两者都是 Scope-LRS v2 transcript profile。`TrustedScope128` 保留的是 128-bit 假设，不是 `LegacyV1`：Scope-LRS v2 修改了 B-link/Fiat--Shamir domain separator 并吸收 profile id，所以仓库早期实验性 v1 矩阵、key image 和签名不保证逐字节兼容。这不影响论文对齐应用：ReSolveD+、RingSig、GroupSig 和 FDABS 仍调用零 IV 的 `StreamingMatrixCols::new`，它们的矩阵 PRG 字节和 transcript 保持不变。

### 14.2 代数关系不变，变的只是公开矩阵派生

两个 profile 都证明同一条关系：

$$
I=B_{\mathrm{link}}\operatorname{RE}(x).
$$

双 profile 共用相同的 witness layout、regular encoding、Merkle 约束、link 约束和 degree-$9$ proof。实际规模为：

| 配置 | witness bits | constraints | degree | canonical wire |
|---|---:|---:|---:|---:|
| Scope-LRS, $\ell=5$ | 15,365 | 8,960 | 9 | 35.06 KiB |
| Scope-LRS, $\ell=10$ | 28,170 | 15,360 | 9 | 60.08 KiB |
| Batch Scope-LRS, $N=10,\ell=5$ | 153,650 | 89,600 | 9 | 306.64 KiB |

实现层面的必要收口如下：

1. `StreamingMatrixCols` 保留旧 `seed + zero IV` 构造器，新增 256-bit `key + IV` 材料路径；AES-NI 和 FAEST FFI 都按同一个 FAEST PRG 定义处理 upper-word tweak 与 low-word block counter。
2. 通用 `Transcript::absorb_matrix` 在 IV 非零时吸收 `label.iv`，在 IV 为零时不增加任何 frame；因此新矩阵状态不会漏绑，旧 transcript 也不变。
3. ReSolveD+ artifact bundle v1 只序列化 128-bit seed，因此现在明确拒绝非零 IV，而不是编码后静默解码成另一个矩阵。
4. `scope_matrix_profile`、scope id、矩阵 key/IV 和 key image 都受 statement transcript 绑定；单签和 Batch 都有交叉 profile 验证失败测试。
5. `signatures_linked` 现在同时比较 profile、规范 scope bytes 和 key image，避免将两个 profile 的同名 scope 当成同一个链接命名空间。

无 profile 参数的公开构造函数默认到 `ChosenScope256`，但这只是安全的 API 默认值，不是对完整 Scope-LRS 安全定理的代替。

### 14.3 所有性能入口统一落到 `justfile`

面向使用者和审稿人的性能入口只是 `just` recipe：

| 公开入口 | 作用 | 双 profile 覆盖 |
|---|---|---|
| `just speed-table` | 固定 P-core 单核的端到端 Sign+encode / decode+Verify | 单签 $\ell=5,10$ 和 Batch $N=10,\ell=5$ 均分别报告 Trusted/Chosen |
| `just speed-table-all-cores` / `just speed-table-both` / `just speed-table-no-hw` | 全核、单/全核合并、portable 对照 | 复用同一张双 profile schema |
| `just bench-matrix-prg` | 固定 P-core 的矩阵 PRG 微基准 | 同轮注册 seed128 与 key/IV-256 两组 Criterion 函数 |
| `just bench-single` / `just bench-parallel` | 冻结的 51 项 Atomic schema | 为不破坏历史 schema，Atomic Scope-LRS 显式固定 `ChosenScope256`；跨 profile 比较以上两个入口为准 |
| `just report` | 小参数/底层诊断 | 已纳入原本只能单独触发的 Rust-vs-C `vole_hash` 性能审计 |

`scripts/paper_speed_table.sh` 与 `scripts/paper_speed_compare.sh` 现在只是 `justfile` 内部的采集/报表辅助程序；没有 `CBZK_PERF_VIA_JUST=1` 时直接执行会以状态 2 退出。三个 Criterion binary 和所有 ignored Rust 计时报告也执行同一门禁，裸 `cargo bench`/`cargo test --ignored` 不再是旁路性能入口。`bench_table.py` 允许直接读取已有 JSON，因为它只渲染、不启动采样。

速度表会保存 `source-status.txt`、worktree/index patch，记录运行前/后 worktree fingerprint、Cargo.lock/子模块状态、每个 sample 实际使用且必须一致的 paper-report binary/hash、Keccak backend 与 native-C backend；采集期间源码或二进制发生变化会使整个 `just` 命令失败。`just speed-table-both` 还要求单核/全核两轮恰好包含相同的 13 个 case、wire size 一致，并逐项比较源码、patch、binary、编译参数与 backend metadata，不再对两张不完整表静默取交集。回归测试覆盖直接入口拒绝、子任务失败传播、双 profile schema、provenance 保存、缺行和跨轮源码不一致拒绝。

### 14.4 双 profile 性能数据及其可解释边界

开发轮通过下列唯一公开入口运行：

```text
PAPER_SPEED_ALLOW_DIRTY=1 \
PAPER_SPEED_REPEATS=10 \
PAPER_SPEED_OUT_DIR=target/paper_speed_table_scope_profiles_final \
just speed-table
```

单核 CPU 2、Rayon 1 worker、10 轮各去掉一个最低/最高值后的结果为：

> **Provenance 限制：** 该目录的 metadata 记录了 `HEAD=e041b0f`、`dirty=true` 和环境，但旧 speed-table helper 未冻结 worktree fingerprint、source patch 或 benchmark binary hash。测量后的改动经逐项核对为文档、标签/注释、回归测试和未激活的旧粗粒度输出，没有修改被测的 Scope-LRS timed relation/PRG/transcript 路径；但这仍不能使它成为“最终 worktree 逐字节可证明”的发布数据。当前门禁/provenance 脚本状态另以 `target/paper_speed_table_scope_profiles_provenance_smoke_final/` 完成 3 轮/13 行 smoke：运行前后源码指纹均为 `1d769570700962729a7201daf9b39a416a10c8d160977341d32f8e042940680a`，binary SHA-256 为 `31402bbc7075dfbf80ed12b1bb1fad049d2970aa8a4903465762cfe27e356b7a`，实际 backend 为 `avx2-xkcp-asm` 与 native-C。这里不用三轮的 middle-one 值代替十轮数据；投稿数据仍必须在 clean commit 上重跑并归档全部证据。

| 配置 | Sign+encode | decode+Verify | wire | timed boundary |
|---|---:|---:|---:|---|
| Trusted128, $\ell=5$ | 93.774 ms | 47.609 ms | 35.06 KiB | scoped Sign 包含 key-image 生成 |
| Chosen256, $\ell=5$ | 86.367 ms | 52.074 ms | 35.06 KiB | 同上 |
| Trusted128, $\ell=10$ | 151.975 ms | 79.789 ms | 60.08 KiB | 同上 |
| Chosen256, $\ell=10$ | 149.386 ms | 89.103 ms | 60.08 KiB | 同上 |
| Batch Trusted128, $N=10,\ell=5$ | 783.372 ms | 451.501 ms | 306.64 KiB | 旧 Batch API；key image 在计时前生成 |
| Batch Chosen256, $N=10,\ell=5$ | 737.818 ms | 422.673 ms | 306.64 KiB | 同上 |

这些端到端差值的方向并不一致：Chosen 的 Sign 表面更快，但单签 Verify 又更慢；原始样本还包含多个远高于中位区间的调度尖峰。两个 profile 的主导 relation/PRG hot loop 相同；Chosen 只额外吸收一个 16-byte IV transcript frame。这一极小固定差异无法解释上表方向相反的数十毫秒波动，因此不应把跨行差值当作 profile 性能因果。它们证明两个入口都可执行并产生同尺寸 wire，不是严格配对 A/B。

更接近机制差异的 `just bench-matrix-prg` 每组采样 100 次。Criterion 终端 `time` 中心值是 `slope.point_estimate`，不是 JSON 里的 `mean.point_estimate`；两种估计量分开披露如下：

| 矩阵宽度 | seed128 slope | key/IV-256 slope | slope 差值 | seed128 mean | key/IV-256 mean | mean 差值 |
|---|---:|---:|---:|---:|---:|---:|
| 4,096 columns | 44.207 us | 45.003 us | +1.80% | 44.579 us | 45.085 us | +1.13% |
| 1,024 columns | 11.397 us | 11.502 us | +0.91% | 11.929 us | 11.352 us | -4.84% |

门禁收口后的最终 `just bench-matrix-prg` 诊断轮更新了上表 JSON；不同估计量在 1,024 列上给出相反方向，且每组存在较多高端 outlier，正说明不能宣称某个 profile 更快。所有差值的绝对值为 0.91%--4.84%，没有发现大幅性能损失，因此没有触发“若 Chosen profile 明显降速则回退”的条件。

### 14.5 与论文的一致性以及 `diff.txt`

根目录现在存在已跟踪的 `diff.txt`（SHA-256 `1d769570700962729a7201daf9b39a416a10c8d160977341d32f8e042940680a`）。逐字验证表明，它等于 `git diff 7b0a466^ 7b0a466 -- . ':(exclude)diff.txt'`：这是 commit `7b0a466` 的自存档历史 patch，不是“与论文原文的 diff”，也不是当前 working-tree diff；它不包含第 14.8 节的 GroupSig Setup 测量修改。当前审查因此同时对照了该历史 patch、当前 `git diff HEAD`、ASIACRYPT 2024 / ePrint 2024/1414 原文参数和本地 raw logs/metadata。若将 `diff.txt` 作为投稿 artifact 发布，必须在附近 README/manifest 明示 base/target commit；直接修改该文件会破坏它与历史 commit patch 的逐字等价。

结论必须分两层：

- 对 ASIACRYPT 2024 原论文应用：没有改变 ReSolveD+、RingSig、GroupSig、FDABS 的 relation、witness、参数、proof 数或零-IV 矩阵字节；通用 transcript 对零 IV 也保持旧 frame。本轮没有新增论文不符。
- 对仓库扩展 Scope-LRS：v2 domain、profile 和非零 IV 是新方案定义，本来就不在原论文 Tables 2--7 中；它们不能被写成“复现论文的 LRS”，也不能借用原 RingSig 定理自动得到 chosen-scope unlinkability。
- 对 Batch 扩展：当前是一份 proof 证明拼接后的直积 relation，应对该合并 relation 应用原 soundness theorem；源码已删除未经组合定理支持的 “$N\epsilon$ amplified bound” 声称，不能凭实例数自行乘一个误差项。

`diff.txt` 对 Scope-LRS/Batch 的“仓库/工程扩展”标签是正确的，其 `n=1280,c=8`、GroupSig `n_e=4096,k_e=3328,t_e=64`、ReSolveD+ `n=453,c=3` 以及 FDABS `L=128,K=512,k=1536` 也与论文文本参数一致。但它保留了两个不应原样进入新论文的历史措辞：

1. “paper-aligned zero-IV matrix bytes” 超过了原文依据；原论文规定公开均匀随机矩阵，没有规定 seed/AES-CTR/IV 字节派生。当前源码和文档已收紧为“本仓库既有矩阵实例化保持逐字节不变”，不再写成论文规定的 encoding。
2. 历史 speed-table 的 `Security parameters` 列只列了 relation 参数，不足以区分 VOLE profile。当前报表已改称 `Relation parameters`，并明示 ReSolveD+ 使用 `ReSolveD128Var2, tau=10, w_grind=8, T_open=102`，其余行使用 `Faest128F, tau=16, w_grind=8, T_open=110`；原始 log 仍逐行打印实际 profile。

第 13.4 节列出的既有 PoC/论文边界仍全部保留，特别是 degree-9 soundness 不应标成 strict 128-bit，以及论文没有本仓库 CPU 的 runtime 表。

### 14.6 历史遗留与尚未完整的性能对照

下列代码此次没有冒充成完整结果：

1. `tests/paper_report.rs::future_report_lrs_single10_ell5` 只是 `#[allow(dead_code)]` 的保留函数，不是 test，也没有 `just` 入口。它还调用预计算 key image 的旧 `sign_lrs` API，且没有 RSS 测量；此次已把原来的“Peak memory O(1)”改成“只能说 B-link 列不物化”。因此它仍不能直接作为 Batch 强 baseline。
2. Batch Scope-LRS 仍是一个 prover 持有全部 witness 的静态直积摊销，不是独立签名者的 public aggregation。它的正式速度行已清楚标记 key image 预计算，但尚无 signer-independent scoped Batch lifecycle API。
3. 冻结的 51 项 Atomic Criterion schema 显式固定 `ChosenScope256`，metadata 与生成表也明确写出该 profile。若未来要把两个 profile 都纳入 Atomic 表，必须同时升级 `bench_table.py`、完整性测试、历史 schema 编号和文档，不能悄悄多加几行。
4. `benches/paper_benchmarks.rs` 约 425--995 行仍保留早期粗粒度 suite，但 canonical `just bench-single/parallel` 强制 `BENCH_SUITE=atomic`，这些函数会在构造昂贵 fixture 前返回；源码已标为 legacy/dead，当前没有性能入口。`BENCH_FILTER=` 只取消 Atomic 名称过滤，不会错误地重新启用它们。
5. `just bench-gf128` 和 `just bench-matrix-prg` 是门禁后的机制诊断入口，但仍使用 Criterion 的 microbenchmark 口径，不具备端到端速度表的完整 source-patch/跨轮发布校验；第 14.4 节只用后者排除明显回退，不把它提升为最终应用性能表。
6. `docs/工程优化PERFORMANCE_OPTIMIZATION_REPORT_CN_2026-07-10.md` 的 §8.4、`docs/[历史遗留]最终性能与优化总结_20260709.md`、模块 12/`原始数据.txt` 的原始日志仍包含裸 `perf stat`、旧 binary 或直接 Criterion 历史命令。模块 11/12 已加现行 `just` 入口或历史免责声明；原始日志保留作为证据，不是当前入口。
7. 固定的第三方 `faest-ref/bench.sh` 属于上游子模块且根仓库从不调用；它还把 `git_commit` 错填成 timestamp，不能作为本 artifact 的发布证据。此次不修改 pinned submodule，只在最终遗留清单中排除它。

上述遗留都不会被当前公开 recipe 静默调用。前两项是真正的功能/比较边界，最终投稿前应当补齐或从主张中降级，而不是仅修改文案；其余项要么是诊断口径，要么是明确隔离的历史/第三方材料。
...
```text
just fmt-check:    passed
just script-tests: 19 passed
just test:         124 passed, 0 failed, 10 ignored; integration tests passed
just test-no-hw:   portable/no-hardware path passed
just paper-tables: passed
just bench-check:  passed
just report:       passed; all low-level performance reports enter through just
just speed-table:  10-repeat development table complete; current provenance path also passed a 3-repeat/13-row smoke
just bench-matrix-prg: completed on AES-NI P-core
mdbook build:      passed
git diff --check:  passed
```

关键新回归包括：非零 IV 与 FAEST FFI 逐字节等价；零 IV transcript 保持不变；非零 IV 必须被吸收；artifact bundle v1 拒绝不可表达的非零 IV；两个 profile 均可单签/Batch round-trip 且不能交叉验签；shell、Criterion binary 和 ignored report 不能绕过 `justfile` 直接成为性能入口；单/全核比较缺行、wire 或 provenance 不一致时必须失败。

本轮不需要为双 profile 手写汇编：非零 IV 路径复用同一 AES-NI hot loop，微基准没有发现降速。若未来继续追求极限性能，仍应按第 13.7 节的条件先做 profile-guided 采样和 intrinsics A/B，只在确认 LLVM 生成了可修复的错误调度后，才把小型 kernel 下沉到汇编。

### 14.8 GroupSig Setup 长尾的统计收口

`Atomic/GroupSig_ell5/Setup` 和 `Atomic/GroupSig_ell10/Setup` 的高方差不是“没有预热”。51 项 canonical Criterion 基准原本就共享 3 s 预热；真正原因是 Setup 每次生成两组独立的 McEliece keypair，而且这部分工作量本身是随机的：

1. `group_setup_with_rng` 收到的 Rust RNG 用于外层 ring setup，不控制 PQClean C keypair 内部的 `randombytes()`；在 Linux 上后者来自 OS 随机源。
2. McEliece `operations.c` 会在 `genpoly_gen` 或 `pk_gen` 拒绝当前候选时重新展开 seed 并继续循环；不同迭代因此可以执行不同次数的重试。
3. 多核路径使用 `rayon::join` 并行这两次单线程 keypair；操作系统调度，特别是混合 P/E 核主机上的调度，会进一步放大墙钟时间的长尾。

因此，此次没有删除 outlier，也没有用确定性 seed ABI 把真实拒绝采样替换为人工固定工作量。修改只发生在 benchmark harness 和报表层：两个 GroupSig Setup 都固定为 `SamplingMode::Flat`、50 个 sample 和 5 s measurement target。`Flat` 使每个 sample 使用同一迭代数，便于直接审计归一化后的长尾分布；它不会消除、截断或“修复”真实随机方差。Setup 登记完成后，harness 立即恢复全局 `SamplingMode::Auto`、10 samples 和 5 s measurement；所以其余 49 项，包括 GroupSig KeyGen/Sign/Verify/Open，仍使用原有采样策略和 3 s 预热。

报表对这两项同时读取 Criterion `median.point_estimate` 及其 95% 置信区间，并从 `sample.json` 将每个 sample 的总时间除以迭代数，报告归一化 mean、P95、CV 和 min--max。若一个 sample 含多次迭代，后四者描述的是“sample 内均值”的分布，不是单次 Setup 的原始 P95。报表还会核对 metadata 中的 Flat mode、sample 数和 measurement target，防止将旧口径误当成新口径。

不同轮次的 OS-randomized keypair 没有配对随机输入，所以 before/after 可以并列原始中位数和分布，但不计算看似精确的 Setup speedup/improvement。GroupSig Setup 是一次性随机化阶段，其数值不用于论证主要 Sign/Verify 性能优势；主要在线性能结论因此不会被这个高方差的一次性阶段统计放大或掩盖。

这一收口没有修改 `src/` 中的生产 RNG、McEliece 生成逻辑、GroupSig 代数关系、transcript、参数或 wire，也没有改变 Sign/Verify 热路径。完整性能验收仍只从 `justfile` 进入：`just bench-single` 和 `just bench-parallel` 分别采集单 P-core 与全核的 51 项 canonical Criterion，`just bench-both` 连续完成两者。`just speed-table*` 是另一套端到端口径，不能替代本节的完整 Criterion 验收。
